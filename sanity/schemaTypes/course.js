...
export default {
  ...
  fields: [
    ...
    },
    {
      ...
      type: 'reference',
      to: [{ type: 'member' }],
      weak: false, // Mantido como FORTE (como já está no seu schema)
      ...
      of: [
        {
          type: 'reference',
          to: [{ type: 'lesson' }],
          weak: true, // Mantido como FRACA (como já está no seu schema)
        }
      ],
  ...
  preview: {
    ...
    prepare(selection) {
      const { title, categoryTitle, subCategoryTitle, media, status, creatorName } = selection;
      const subtitleParts = [];
