...
import axios from 'axios'; // <-- NOVA IMPORTAÇÃO: AXIOS para requisições HTTP
...
// --- FUNÇÃO: generateCoursePreview (AGORA COM INTEGRAÇÃO PIXABAY) ---
export const generateCoursePreview = async (req, res) => {
    ...
    try {
        ...
        if (tags && tags.length > 0) {
            // Certifique-se que 'tags' é um array de IDs, se o Sanity espera isso.
            // Se tags vêm do frontend como objetos, ajuste para pegar o ID.
            const tagIds = tags.map(tag => tag._id || tag); // Pega o ID se for objeto, ou usa direto se já for ID
            const tagDetails = await sanityClient.fetch(
                `*[_type == "courseTag" && _id in $tagIds]{name, description}`,
                { tagIds } // Passa os IDs para a query
            );
        ...
        **Varie o início do título do curso e das lições** com diferentes abordagens e sinônimos (ex: "Fluência em Inglês", "Domine o Inglês", "Inglês na Prática", "Guia Completo de Inglês", "Desvende o Inglês", etc. para títulos de curso de inglês). **Evite repetir as mesmas palavras iniciais nos títulos de cursos e lições.**
        ...
        // --- INÍCIO DA NOVA LÓGICA: BUSCA DE IMAGENS DO PIXABAY ---
        let pixabayImages = [];
        try {
            const pixabayApiKey = process.env.PIXABAY_API_KEY;
            
            // Construa a query para o Pixabay usando os dados do curso e as tags (se tags tiverem 'name')
            // Certifique-se que `tags` tem a estrutura esperada (array de objetos com `name` ou array de strings)
            const tagNames = tags.map(tag => typeof tag === 'object' && tag.name ? tag.name : tag).filter(Boolean).join(' ');
            const pixabayQuery = `${topic} ${generatedCourseData.title || ''} ${category} ${subCategory} ${tagNames}`.trim(); 
            
            if (pixabayQuery) { // Só faz a requisição se houver termos de busca
                const pixabayUrl = `https://pixabay.com/api/?key=${pixabayApiKey}&q=${encodeURIComponent(pixabayQuery)}&image_type=photo&orientation=horizontal&per_page=9`; // Buscar 9 para ter mais opções para randomizar

                const pixabayResponse = await axios.get(pixabayUrl);
                const hits = pixabayResponse.data.hits;

                if (hits.length > 0) {
                    const shuffled = hits.sort(() => 0.5 - Math.random()); // Embaralha o array
                    // Seleciona as primeiras 3 imagens após embaralhar
                    pixabayImages = shuffled.slice(0, 3).map(hit => ({
                        id: hit.id,
                        webformatURL: hit.webformatURL,       // URL para exibição de pré-visualização (menor)
                        largeImageURL: hit.largeImageURL,     // URL da imagem maior para salvar no Sanity
                        tags: hit.tags                        // Tags do Pixabay para referência
                    }));
                }
            } else {
                console.warn("[BACKEND] Nenhum termo de busca eficaz para o Pixabay. Pulando busca de imagens.");
            }
        } catch (pixabayError) {
            console.error('Erro ao buscar imagens do Pixabay (não fatal para a pré-visualização):', pixabayError.message);
            // Continua mesmo se a busca do Pixabay falhar, mas as imagens não serão incluídas
        }
        // --- FIM DA NOVA LÓGICA: BUSCA DE IMAGENS DO PIXABAY ---
        ...
        const responseData = {
            ...
            level: level,
            tags: tags, // Enviando os IDs ou objetos de tags de volta
            promptUsed: prompt, 
            aiModelUsed: "gemini-2.0-flash",
            pixabayImages: pixabayImages // <-- ADICIONA AS IMAGENS DO PIXABAY À RESPOSTA AQUI!
        };
...
// --- FUNÇÃO: saveGeneratedCourse (AGORA COM SALVAMENTO DA IMAGEM ESCOLHIDA) ---
export const saveGeneratedCourse = async (req, res) => {
    ...
    const { courseData, category, subCategory, level, tags, selectedImageUrl } = req.body; // <-- RECEBENDO selectedImageUrl AQUI!
    const creatorId = req.user?.id; 
    ...
    console.log('[BACKEND - saveGeneratedCourse] selectedImageUrl:', selectedImageUrl); // <-- LOG DA IMAGEM ESCOLHIDA
    ...
    try {
        ...
        const courseTagRefs = []; 
        if (tags && tags.length > 0) {
            // Certifique-se que 'tags' é um array de IDs ou objetos com '_id'.
            // Mapeie para referências Sanity se necessário.
            tags.forEach(tag => {
                const tagId = typeof tag === 'object' && tag._id ? tag._id : tag;
                if (tagId) { // Garante que o ID não é nulo/vazio
                    courseTagRefs.push({
                        _ref: tagId,
                        _type: 'reference',
                        _key: uuidv4(), 
        ...
        // --- INÍCIO DA NOVA LÓGICA: SALVAMENTO DA IMAGEM DO CURSO NO SANITY ---
        let imageReference = null; // Renomeado para 'imageReference'
        if (selectedImageUrl) {
            try {
                // 1. Baixar a imagem do Pixabay
                const imageResponse = await axios.get(selectedImageUrl, { responseType: 'arraybuffer' });
                const imageBuffer = Buffer.from(imageResponse.data);
                // Tenta pegar o Content-Type do cabeçalho da resposta, ou usa um fallback seguro
                const contentType = imageResponse.headers['content-type'] || 'image/jpeg'; 

                // 2. Fazer upload para o Sanity Asset Management
                const asset = await sanityClient.assets.upload('image', imageBuffer, { contentType: contentType });
                
                // 3. Criar a referência da imagem para o documento do curso
                imageReference = { // Usando 'imageReference' aqui
                    _type: 'image',
                    asset: {
                        _ref: asset._id,
                        _type: 'reference'
                ...
                console.log(`[BACKEND] Imagem Pixabay carregada para Sanity com ID: ${asset._id}`);

            } catch (imageUploadError) {
                console.error('[BACKEND] Erro ao baixar ou fazer upload da imagem do Pixabay para o Sanity (curso será salvo sem imagem):', imageUploadError.message);
                // Não retorna erro fatal; o curso ainda será salvo, mas sem a imagem principal.
                // Você pode querer logar isso ou enviar um aviso ao frontend.
            }
        }
        // --- FIM DA NOVA LÓGICA: SALVAMENTO DA IMAGEM DO CURSO NO SANITY ---
        ...
        const newCourse = {
            ...
            image: imageReference, // <-- CORRIGIDO AQUI PARA 'image'!
        };
